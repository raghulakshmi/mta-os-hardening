...
# File    : 03-mobileum-cis-l1-os-hardening-v1.4.sh
# Version : 1.3
...
# ------------------------------
#   sudo ./03-mobileum-cis-l1-os-hardening-v1.4.sh --check
#   sudo ./03-mobileum-cis-l1-os-hardening-v1.4.sh --apply-safe
#   sudo ./03-mobileum-cis-l1-os-hardening-v1.4.sh --check
#
#   # PAM only after console/SSM recovery path is confirmed:
#   sudo ./03-mobileum-cis-l1-os-hardening-v1.4.sh --apply-pam
#
...
#   sudo ./03-mobileum-cis-l1-os-hardening-v1.4.sh --apply-firewall
#
#   # Optional controlled operations:
#   sudo ./03-mobileum-cis-l1-os-hardening-v1.4.sh --install-security-updates
#   sudo ./03-mobileum-cis-l1-os-hardening-v1.4.sh \
...
usage() {
    cat <<'USAGE'
Usage:
  sudo ./03-mobileum-cis-l1-os-hardening-v1.4.sh [options]
...
Examples:
  ...
  sudo ./03-mobileum-cis-l1-os-hardening-v1.4.sh --apply-firewall --firewall-role mta1
...
# Managed by 03-mobileum-cis-l1-os-hardening-v1.4.sh
#
...
PAM
    ...
    # Password quality uses Ubuntu's package-supplied /usr/share/pam-configs/pwquality
    # profile from libpam-pwquality. Policy values remain in
    # /etc/security/pwquality.conf.d/50-mobileum.conf. Keeping a second custom
    # pwquality PAM profile would duplicate pam_pwquality.so in common-password.
...
remove_legacy_mobileum_pwquality_profile() {
    # v1.3 created a custom pwquality PAM profile in addition to Ubuntu's
    # package-supplied pwquality profile. If upgrading an already-hardened host,
    # explicitly deselect and remove that legacy profile before regeneration.
    local legacy="/usr/share/pam-configs/mobileum-pwquality"
    if [[ -f "$legacy" ]]; then
        info "Removing legacy duplicate Mobileum pwquality PAM profile from v1.3."
        pam-auth-update --force --remove mobileum-pwquality >/dev/null 2>&1 || true
        rm -f "$legacy"
        changed "Legacy duplicate Mobileum pwquality profile removed."
    fi
}
...
validate_pam_stack() {
    ...
    if (( ok == 1 )); then
        pass "PAM stack contains faillock, exactly one pwquality module, and pwhistory; pam_unix nullok is absent and use_authtok/strong hashing are present."
        record_status "pam_stack" "PASS" "faillock/single-pwquality/pwhistory active; pam_unix nullok absent; use_authtok + strong hash present"
        return 0
    fi
...
FLC
    ...
    # Regenerate common-* using the project profiles plus Ubuntu's packaged
    # pwquality profile. --force is used only after the complete PAM backup.
    ...
    # Password-locked cloud accounts (for example the AWS ubuntu account) are
    # intentionally not assigned password expiry because their administrative
    # access is key-based; record this as a project exception rather than risk
    # future key-login disruption from password-expiry/account PAM handling.
...
check_pam() {
    ...
    if (( pam_ok == 1 )); then
        ...
        record_status "pam_check" "PASS" "required modules present exactly once where required; nullok absent"
    else
    ...
    : > "${EVIDENCE_DIR}/account-aging-review.tsv"
    ...
    while IFS=: read -r user _ uid _ _ _ shell; do
        ...
        elif [[ "$mind" == "$PASS_MIN_DAYS" && "$maxd" == "$PASS_MAX_DAYS" && "$warnd" == "$PASS_WARN_AGE" ]]; then
            assessment="PASS"
        else
            assessment="REVIEW"
        fi
        printf '%s\t%s\t%s\t%s\t%s\t%s\n' "$user" "$state" "$mind" "$maxd" "$warnd" "$assessment" >> "${EVIDENCE_DIR}/account-aging-review.tsv"
    done < <(getent passwd)
}
