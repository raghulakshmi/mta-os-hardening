#!/usr/bin/env bash
# ==============================================================================
# Mobileum MTA - 02 Pre-Hardening Assessment v3
...
# Usage
# -----
#   sudo bash 02-mobileum-pre-hardening-assessment-v3.sh
#
# Output
# ------
...
# Default dpkg-query output is package<TAB>version.
# Avoid nested ${...} formatting because this block is executed by bash -c.
dpkg-query -W | sort
'
...
(
    # Evidence-only summary: individual probe failures must not terminate the run.
    set +e
    echo "=============================================================================="
...
EXPECTED_FILES=(
    01-system-info.txt
    02-hardware-storage.txt
    03-packages-installed.txt
    03-apt-sources.txt
    04-updates-security.txt
    05-services.txt
    06-listening-ports.txt
    07-network.txt
    08-users-groups.txt
    09-sudo.txt
    10-ssh.txt
    11-firewall.txt
    12-apparmor.txt
    13-audit-logging.txt
    14-sysctl-security.txt
    15-kernel-modules.txt
    16-pam-password-policy.txt
    17-critical-file-permissions.txt
    18-cron-at.txt
    19-time-sync.txt
    20-boot-security.txt
    21-suid-sgid-world-writable.txt
    22-optional-security-tools.txt
    PRE-HARDENING-SUMMARY.txt
    ASSESSMENT-COMPLETE
)
...
for f in "${EXPECTED_FILES[@]}"; do
    if [[ ! -e "${OUT}/${f}" ]]; then
        printf '[WARN] Missing expected evidence file: %s\n' "$f"
        EVIDENCE_ISSUES=$((EVIDENCE_ISSUES + 1))
    fi
done

PKG_LINES="$(grep -cv '^[[:space:]]*$' "${OUT}/03-packages-installed.txt" 2>/dev/null || true)"
if [[ "${PKG_LINES:-0}" -lt 50 ]]; then
    printf '[WARN] Package inventory looks unexpectedly small (%s non-empty lines).\n' "${PKG_LINES:-0}"
    EVIDENCE_ISSUES=$((EVIDENCE_ISSUES + 1))
else
    printf '[PASS] Package inventory captured (%s non-empty lines).\n' "$PKG_LINES"
fi

if grep -q 'syntax error' "${OUT}/08-users-groups.txt" 2>/dev/null; then
    printf '[WARN] Account inventory contains a syntax error.\n'
    EVIDENCE_ISSUES=$((EVIDENCE_ISSUES + 1))
else
    printf '[PASS] Account inventory completed without syntax errors.\n'
fi

log "Pre-hardening assessment complete."
if [[ "$EVIDENCE_ISSUES" -eq 0 ]]; then
    echo "Evidence completeness: PASS"
else
    echo "Evidence completeness: WARN (${EVIDENCE_ISSUES} issue(s))"
fi
echo
echo "Summary:"
echo "  ${SUMMARY}"
