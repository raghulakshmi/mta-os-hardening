...
# File    : 04-mobileum-post-hardening-validation-v2.sh
# Version : 2.0
...
# * v2 explicitly validates /etc/ufw/sysctl.conf so UFW reloads cannot silently
#   revert net.ipv4.conf.{all,default}.log_martians from 1 back to 0.
# * When a known remediation is identified, exact commands are written to
#   REMEDIATION-COMMANDS.txt. They are never executed by this script.
#
# USAGE
# -----
#   sudo bash 04-mobileum-post-hardening-validation-v2.sh
#
...
REMEDIATION_FILE="${OUT}/REMEDIATION-COMMANDS.txt"
...
usage() {
    cat <<'USAGE'
Usage:
  sudo bash 04-mobileum-post-hardening-validation-v2.sh [options]
...
Mobileum MTA 04 Post-Hardening Validation - Remediation Commands
================================================================

This file is generated by a READ-ONLY validator. Commands are NOT executed
automatically. Review each remediation before running it.
...
write_summary() {
    ...
    chmod 0600 "$SUMMARY" "$RESULT_FILE" "$COMPLETE_MARKER" "$STATUS_TSV" "$COMPARE_TSV" "$REMEDIATION_FILE" 2>/dev/null || true
}
...
check_ufw_sysctl_persistence() {
    info "Checking UFW sysctl persistence for Mobileum martian logging controls."

    local ufw_conf="/etc/ufw/sysctl.conf"
    local runtime_all runtime_default conf_all conf_default
    runtime_all="$(sysctl -n net.ipv4.conf.all.log_martians 2>/dev/null || echo N/A)"
    runtime_default="$(sysctl -n net.ipv4.conf.default.log_martians 2>/dev/null || echo N/A)"

    if [[ ! -f "$ufw_conf" ]]; then
        warn "${ufw_conf} is not present; no UFW sysctl override could be validated."
        record_status "ufw_sysctl_persistence" "WARN" "${ufw_conf} missing"
        return
    fi

    # UFW uses slash-separated sysctl keys in this file. Accept dotted keys too
    # in case the administrator has normalized the syntax manually.
    conf_all="$(awk -F= '
      /^[[:space:]]*(net\/ipv4\/conf\/all\/log_martians|net\.ipv4\.conf\.all\.log_martians)[[:space:]]*=/ {
        v=$2; gsub(/[[:space:]]/,"",v); print v; exit
      }' "$ufw_conf" 2>/dev/null || true)"
    conf_default="$(awk -F= '
      /^[[:space:]]*(net\/ipv4\/conf\/default\/log_martians|net\.ipv4\.conf\.default\.log_martians)[[:space:]]*=/ {
        v=$2; gsub(/[[:space:]]/,"",v); print v; exit
      }' "$ufw_conf" 2>/dev/null || true)"

    capture_shell "03b-ufw-sysctl-persistence.txt" "printf 'UFW sysctl file: %s\\n' '$ufw_conf'; grep -nE 'log_martians' '$ufw_conf' 2>/dev/null || true; echo; sysctl net.ipv4.conf.all.log_martians net.ipv4.conf.default.log_martians 2>/dev/null || true"

    if [[ "$conf_all" == "1" && "$conf_default" == "1" ]]; then
        pass "UFW sysctl persistence matches Mobileum baseline: all/default log_martians=1."
        record_status "ufw_sysctl_persistence" "PASS" "all=1 default=1"
        return
    fi

    fail "UFW sysctl persistence conflicts with Mobileum baseline: ${ufw_conf} all=${conf_all:-UNSET}, default=${conf_default:-UNSET}; runtime all=${runtime_all}, default=${runtime_default}. A UFW reload can reset log_martians."
    record_status "ufw_sysctl_persistence" "FAIL" "file all=${conf_all:-UNSET} default=${conf_default:-UNSET}; runtime all=${runtime_all} default=${runtime_default}"

    add_remediation "Fix UFW log_martians persistence" \
"# Keep the current SSH administrative session open while applying this change." \
"sudo cp -a /etc/ufw/sysctl.conf /etc/ufw/sysctl.conf.pre-mobileum-\$(date -u +%Y%m%dT%H%M%SZ)" \
"sudo sed -ri 's|^[[:space:]]*(net/ipv4/conf/all/log_martians|net\\.ipv4\\.conf\\.all\\.log_martians)[[:space:]]*=.*$|net/ipv4/conf/all/log_martians=1|' /etc/ufw/sysctl.conf" \
"grep -Eq '^[[:space:]]*(net/ipv4/conf/all/log_martians|net\\.ipv4\\.conf\\.all\\.log_martians)[[:space:]]*=' /etc/ufw/sysctl.conf || echo 'net/ipv4/conf/all/log_martians=1' | sudo tee -a /etc/ufw/sysctl.conf >/dev/null" \
"sudo sed -ri 's|^[[:space:]]*(net/ipv4/conf/default/log_martians|net\\.ipv4\\.conf\\.default\\.log_martians)[[:space:]]*=.*$|net/ipv4/conf/default/log_martians=1|' /etc/ufw/sysctl.conf" \
"grep -Eq '^[[:space:]]*(net/ipv4/conf/default/log_martians|net\\.ipv4\\.conf\\.default\\.log_martians)[[:space:]]*=' /etc/ufw/sysctl.conf || echo 'net/ipv4/conf/default/log_martians=1' | sudo tee -a /etc/ufw/sysctl.conf >/dev/null" \
"sudo grep -n 'log_martians' /etc/ufw/sysctl.conf" \
"sudo ufw reload" \
...
"sudo bash 04-mobileum-post-hardening-validation-v2.sh"
    ...
    warn "Exact remediation commands were written to ${REMEDIATION_FILE}."
}
...
main() {
    ...
    echo "Remediation: ${REMEDIATION_FILE}"
    echo "=============================================================================="
